...
public class TestNodeRessource {
	...
	public void registerNode() {
		...
		if (response == null) {
			throw new RuntimeException("Couldn't register at ManagementServer, response is null!");
		}
		...
		/*
		 * List<Node> nodes = NodeDAO.getNodes(); Assert.assertNotNull(
		 * "getNodes() returned NULL after successfull node registration."
		 * ,nodes); Assert.assertFalse(
		 * "getNodes() returned empty list after successfull node registration."
		 * , nodes.isEmpty());
		 */

		// UNREGISTER:

		LOGGER.info("Sending unregistration request with NodeID (" + nodeId + ") to ManagemetServer!");
		// unregister
